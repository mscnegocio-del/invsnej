...
| `/agente-v2` (HUD/Jarvis) como ruta paralela, no reemplaza `/agente` | 2026-07-20 | Migrar `/agente` directamente al nuevo diseño | Permite comparar en vivo y decidir con uso real antes de comprometerse a un cambio de identidad visual |
| `/agente-v2` reutiliza los hooks reales (`useAIChat`, `useSpeechRecognition`) y solo cambia la piel (CSS aislado bajo `.av2`) | 2026-07-20 | Duplicar lógica de chat/voz/confirmación para el prototipo | Evita mantener dos implementaciones del mismo comportamiento; si se adopta, solo hay que retemar, no reescribir |
| Home gana una tarjeta destacada que enlaza a `/agente` (v1, no v2) | 2026-07-20 | Enlazar a v2, o dar a elegir entre v1/v2 desde Home | v2 sigue en pruebas; el Agente no puede quedar escondido solo en el menú lateral si se espera que sea la vía principal de uso |
| Se elimina el sidebar/bottom-nav fijo; Home es el único hub y ⌘K reemplaza la navegación rápida de escritorio | 2026-07-21 | Mantener el menú fijo junto al nuevo hub de Home | Con 8 secciones el menú duplicaba a Home y generaba "contaminación visual" (feedback directo del usuario); el botón "Inicio" explícito + ⌘K cubre tanto el flujo táctil como el de teclado sin redundancia |
| El botón "volver" pasa de `navigate(-1)` (historial) a un Link explícito a `/` | 2026-07-21 | Mantener el back basado en historial | El pedido explícito era "un botón visible para volver al menú inicio" — un destino fijo y predecible, no dependiente de por dónde entró el usuario |
